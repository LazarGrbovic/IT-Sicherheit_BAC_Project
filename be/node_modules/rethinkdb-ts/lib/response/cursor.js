...
class Cursor extends stream_1.Readable {
    ...
    _read() {
        ...
        this._next()
            .then(push)
            .catch((err) => {
            if ((!error_1.isRethinkDBError(err) ||
                ![
    ...
    async each(callback, onFinishedCallback) {
        ...
        if (this.closed) {
            callback(new error_1.RethinkDBError('You cannot retrieve data from a cursor that is closed', { type: types_1.RethinkDBErrorType.CURSOR }));
        ...
        try {
            while (!this.closed) {
                ...
                if (rowHandler.length > 1) {
                    ...
                    if (result !== undefined && !util_1.isPromise(result)) {
                        throw result;
                    }
                    await result;
                }
            ...
            if (!error_1.isRethinkDBError(error) ||
        ...
        return {
            next: async () => {
                ...
                try {
                    ...
                    if (error_1.isRethinkDBError(error) &&
                        error.type === types_1.RethinkDBErrorType.CANCEL) {
                        return { done: true, value: undefined };
                    }
                    throw error;
                }
